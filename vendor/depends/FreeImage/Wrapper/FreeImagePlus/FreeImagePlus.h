...
// 看图王集成BEGIN
#ifndef __ILoadImageCallBack_
#define __ILoadImageCallBack_ 1
...
public:
    ...
    */
    ...
    /** 设置进度
    */
    virtual void SetProgress(uint64_t completed, uint64_t total);

protected:

    /** 析构函数
    */
    virtual ~ILoadImageCallBack() {};
};

#endif // __ILoadImageCallBack_
...
struct fipFrameInfo
{
    ...
    /// height
    int h = 0;
    /// Dispose method(value of WebPMuxAnimDispose) [0:Do not dispose, 1:Dispose to background color]
    int dispose = 0;
    /// Blend operation(value of WebPMuxAnimBlend) [0:Blend, 1:Do not blend]
    int blend = 0;
    /// Duration
    int duration = 100;
};
...
protected:
    ...
    /// MultiPage
    fipMultiPage* _multiDib;

    /// current frame info
    fipFrameInfo _curFrameInfo;

    /// previous frame info
    fipFrameInfo _prevFrameInfo;
...
public:
	...
	*/
	BOOL load(const char* lpszPathName, int flag, ILoadImageCallBack* callback);
	...
	*/
	BOOL load(FREE_IMAGE_FORMAT fif, const char* lpszPathName, int flag, ILoadImageCallBack* callback);

	/**
	UNICODE version of load (this function only works under WIN32 and does nothing on other OS)
	@see load
	*/
	BOOL loadU(const wchar_t* lpszPathName, int flag, ILoadImageCallBack* callback);

	/**
	UNICODE version of load (this function only works under WIN32 and does nothing on other OS)
	@see load
	*/
	BOOL loadU(FREE_IMAGE_FORMAT fif, const wchar_t* lpszPathName, int flag, ILoadImageCallBack* callback);
	...
	*/
	BOOL loadFromHandle(FreeImageIO *io, fi_handle handle, int flag, ILoadImageCallBack* callback);
	...
	*/
	BOOL loadFromMemory(fipMemoryIO& memIO, int flag, ILoadImageCallBack* callback);
	...
	@see FreeImage_LoadFromMemory, FreeImage documentation
	*/
	BOOL loadFromMemory(FREE_IMAGE_FORMAT fif, fipMemoryIO& memIO, int flag, ILoadImageCallBack* callback);
	...
	*/
    ...
    fipMultiPage* GetFipMultiPage() {
        return _multiDib;
    }

    /** get show frame of multi-page bitmap
    */
    FIBITMAP* GetFrame(int page);

    /** get original frame of multi-page bitmap
    */
    FIBITMAP* GetOriginalFrame(fipFrameInfo& info, int page);

    /** active frame of multi-page bitmap
    */
    BOOL ActiveFrame(int page);

    /** get frame duration of multi-page bitmap
    */
...
public :
	...
	*/
    FIBITMAP* load(FREE_IMAGE_FORMAT fif, int flags = 0, ILoadImageCallBack* callback = 0) const;
	/**
...
protected:
    ...
    /// key frame of WebP
    std::map<int, fipKeyFrame>* _mapKeyFrame;

    /// WebP memory io, when size < 20M
    fipMemoryIO* _memIO;
...
public:
	...
	*/
    ...
    @see FreeImage_AppendPage
    */
    void appendKeyPage(const fipFrameInfo& info, FIBITMAP* dib, int page);

    /**
    @param page Page number
    @return Returns the page if the page is key-frame page
    */
    FIBITMAP* getKeyPage(fipFrameInfo& info, int page);
};
