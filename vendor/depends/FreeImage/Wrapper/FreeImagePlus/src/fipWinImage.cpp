...
HANDLE fipWinImage::copyToHandle() const {
	...
	if(_dib) {
        ...
        if (FreeImage_IsTransparent(_dib) && FreeImage_GetBPP(_dib) == 32)
        {
            int width = FreeImage_GetWidth(_dib);
            int height = FreeImage_GetHeight(_dib);
            int pitch = FreeImage_GetPitch(_dib);
            for (int y = 0; y < height; y++)
            {
                BYTE *bits = p_dib + pitch * y;

                for (int x = 0; x < width; x++)
                {
                    const BYTE alpha = bits[FI_RGBA_ALPHA];
                    BYTE not_alpha = (BYTE)~alpha;
                    bits[FI_RGBA_RED] = (BYTE)(((alpha * bits[FI_RGBA_RED]) >> 8) + not_alpha);
                    bits[FI_RGBA_GREEN] = (BYTE)(((alpha * bits[FI_RGBA_GREEN]) >> 8) + not_alpha);
                    bits[FI_RGBA_BLUE] = (BYTE)(((alpha * bits[FI_RGBA_BLUE]) >> 8) + not_alpha);
...
void fipWinImage::drawEx(HDC hDC, RECT& rcDest, RECT& rcSrc, BOOL useFileBkg, RGBQUAD *appBkColor, FIBITMAP *bg) const {
    ...
    bool enableICMMode = false;

    BITMAPINFO* pBitmapICCHeader = NULL;
    FIICCPROFILE* profile = FreeImage_GetICCProfile(_dib);
    if (profile != NULL)
    {
        bool isCMYKColorSpace = ((profile->flags & FIICC_COLOR_IS_CMYK) == FIICC_COLOR_IS_CMYK);
        if (isCMYKColorSpace && (profile->size > 0))
        {
            pBitmapICCHeader = FreeImage_CreateICCProfileHeaderData(_display_dib, profile->data, profile->size);
            enableICMMode = true;
        }
    }
    ...
    BITMAPINFO* pBitmaHeader = ((pBitmapICCHeader != NULL) ? pBitmapICCHeader : (BITMAPINFO*)FreeImage_GetInfoHeader(_display_dib));
    ...
    if (pBitmapICCHeader != NULL)
    {
        FreeImage_DestroyICCProfileHeaderData(pBitmapICCHeader);
        pBitmapICCHeader = NULL;
    }
