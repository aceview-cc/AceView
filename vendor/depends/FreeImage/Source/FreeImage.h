...
// 看图王集成BEGIN
// Load Call ----------------------------------------------------------------
typedef void (DLL_CALLCONV *PFN_RCLoadImage_OnInit)(void* handle, void* data);
typedef void (DLL_CALLCONV *PFN_RCLoadImage_SetBreak)(void* handle);
typedef BOOL(DLL_CALLCONV *PFN_RCLoadImage_CheckBreak)(void* handle);
typedef void (DLL_CALLCONV *PFN_RCLoadImage_SetProgress)(void* handle, uint64_t completed, uint64_t total);

FI_STRUCT(FreeImage_LoadCallBack) {
    PFN_RCLoadImage_OnInit         OnInitProc;
    //PFN_RCLoadImage_SetBreak     SetBreakProc;
    PFN_RCLoadImage_CheckBreak     CheckBreakProc;
    PFN_RCLoadImage_SetProgress    SetProgressProc;
    void*  pHandle;
};
...
typedef FIBITMAP *(DLL_CALLCONV *FI_LoadProc)(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack* callback);
...
#define JPEG_NOT_USEICC     0x0020  //! load and not convert use ICC
...
#define JPEG_QUALITYCROP    0x4000	// 裁剪用质量 (95:1)
#define JPEG_QUALITYFREE    0x8000	// 自定义质量,以数字计
...
#define PNG_IGNOREMULTIPLE  2       //！loading: avoid png multiple 
...
#define PSD_NOT_USEICC      4       //! PSD not use ICC conversion
...
// 看图王集成所在修改BEGIN
#define LOAD_CHECK_BREAK_BEGIN \
    BOOL bCheck = (callback != NULL) && (callback->CheckBreakProc != NULL );

#define LOAD_CHECK_STEP  ~7

// 加载进度-检查行数
#define PROGRESS_STEP   50  

#define LOAD_THROW throw (const char*)NULL; 

#define LOAD_CHECK_BREAK(line, deal)                                                                        \
    if (bCheck && ((line & LOAD_CHECK_STEP) ==  line) && callback->CheckBreakProc(callback->pHandle) )      \
        {                                                                                                       \
        deal;                                                                                               \
        };
// 看图王集成所在修改END
...
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_Load(FREE_IMAGE_FORMAT fif, const char *filename, int flags, FreeImage_LoadCallBack* callback);
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_LoadU(FREE_IMAGE_FORMAT fif, const wchar_t *filename, int flags, FreeImage_LoadCallBack* callback);
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_LoadFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flags, FreeImage_LoadCallBack* callback);
...
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_LoadFromMemory(FREE_IMAGE_FORMAT fif, FIMEMORY *stream, int flags, FreeImage_LoadCallBack* callback);
DLL_API BOOL DLL_CALLCONV FreeImage_SaveToMemory(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, FIMEMORY *stream, int flags FI_DEFAULT(0));
DLL_API BOOL DLL_CALLCONV FreeImage_FakeSaveToMemory(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, FIMEMORY *stream, int flags FI_DEFAULT(0));
...
DLL_API FIBITMAP* DLL_CALLCONV FreeImage_LockPageByHandle(FIMULTIBITMAP* bitmap, int page);
...
DLL_API BITMAPINFO* DLL_CALLCONV FreeImage_CreateICCProfileHeaderData(FIBITMAP* dib, void* data, long size);
DLL_API void DLL_CALLCONV FreeImage_DestroyICCProfileHeaderData(BITMAPINFO* pICCHeader);
DLL_API BOOL DLL_CALLCONV FreeImage_CopyICCProfile(FIBITMAP* destDIB, FIBITMAP* srcDIB);
...
DLL_API BOOL DLL_CALLCONV FreeImage_Paste(FIBITMAP *dst, FIBITMAP *src, int left, int top, int alpha, BOOL modifyDstAlpha FI_DEFAULT(TRUE));
