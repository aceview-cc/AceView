...
*/
static FIBITMAP * 
libraw_ConvertProcessedRawToDib(LibRaw *RawProcessor, FreeImage_LoadCallBack * callback = NULL) {
	FIBITMAP *dib = NULL;
	...
	try {
		...
		if(thumb_image) {
			if(thumb_image->type != LIBRAW_IMAGE_BITMAP) {
				...
				dib = FreeImage_LoadFromMemory(fif, hmem, flags, NULL);
				// close the stream
...
*/
static FIBITMAP * 
libraw_LoadRawData(LibRaw *RawProcessor, int bitspersample, FreeImage_LoadCallBack * callback) {
	FIBITMAP *dib = NULL;
	...
	try {
		...
		dib = libraw_ConvertProcessedRawToDib(RawProcessor, callback);
...
// 看图王集成BEGIN
class CLibRowCheckBreak : public ILibRowCheckBreak
{
public:
    CLibRowCheckBreak(FreeImage_LoadCallBack * callback) :m_pCallback(callback)
    {
        if ((callback != NULL) && (callback->CheckBreakProc != NULL))
        {
    ...
    virtual BOOL CheckBreak()
    {
        if (m_pCallback != NULL)
        {
            return m_pCallback->CheckBreakProc(m_pCallback->pHandle);
        }
        else
        {
            return FALSE;
...
private:
    ...
    FreeImage_LoadCallBack * m_pCallback;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack * callback) {
	...
	try {
        ...
        CLibRowCheckBreak check(callback);

        if ((callback != NULL) && (callback->CheckBreakProc != NULL))
        {
            RawProcessor->m_pCheckBreak = &check;
        }
		...
		if(header_only) {
			...
			dib = libraw_LoadRawData(RawProcessor, 8, callback);
		} 
		else {
			// default: load raw data as linear 16-bit/sample (i.e. RGB 48-bit)
			dib = libraw_LoadRawData(RawProcessor, 16, callback);
		}
