...
// 看图王集成BEGIN
class CPngCheckBreak
{
public:
    CPngCheckBreak(FreeImage_LoadCallBack * callback)
        :m_Callback(callback)
    {
        m_Check.PNG_RCLoadImage_CheckBreak = CheckBread;
    }
    static int CheckBread(void *object, void* handle)
    {
        CPngCheckBreak *p = (CPngCheckBreak *)object;
        return p->m_Callback->CheckBreakProc(handle);
    }

public:

    IPngCheckBreak m_Check;

    FreeImage_LoadCallBack * m_Callback;
};
// 看图王集成END
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack* callback) {
	...
	if (handle) {
		...
		try {		
            ...
            {
                //APNG 直接返回用另外的解码方式
                if ((flags & PNG_IGNOREMULTIPLE) == PNG_IGNOREMULTIPLE)
                {
            ...
            for (png_uint_32 k = 0; k < height; k++) {
                LOAD_CHECK_BREAK(k, LOAD_THROW);

                if ((k % PROGRESS_STEP == 0) && callback && (callback->SetProgressProc != NULL))
                {
                    callback->SetProgressProc(callback->pHandle, k, height);
                }
            ...
            CPngCheckBreak check(callback);
            if (png_read_image(png_ptr, row_pointers, (bCheck) ? &check.m_Check : NULL, (bCheck) ? callback->pHandle : NULL) != 0)
            {
                LOAD_THROW;
            }
