...
// 看图王集成BEGIN
static BYTE
Read8Check(FreeImageIO *io, fi_handle handle, BYTE i) {
    io->read_proc(&i, 1, 1, handle);
    return i;
}
// 看图王集成END
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack * callback) {
	...
	try {		
        ...
        //由于读取的返回值与IO失败的返回值都是0，这里会有死循环
        BYTE b = 0;
        while ((b = Read8Check(io, handle, 0xFF)) == 0);
		if ( b != 0x11 ) {
...
//					found = TRUE;
		...
		switch ( pictType )	{
			...
			case jpeg:
			{
				dib = FreeImage_LoadFromHandle( FIF_JPEG, io, handle, 0, NULL );					
				break;
			}
