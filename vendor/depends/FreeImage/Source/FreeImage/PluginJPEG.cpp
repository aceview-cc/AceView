...
// 看图王集成BEGIN
#include "../../../../include/lcms/lcms2.h"

// 全局Profile文件 大小 与 内容
extern DWORD s_dIccProfileSize;
extern void* s_pIccProfileData;
// 看图王集成END
...
#define INPUT_BUF_SIZE  (4096 * 8)  // choose an efficiently fread'able size 
#define OUTPUT_BUF_SIZE (4096 * 8)  // choose an efficiently fwrite'able size
...
jpeg_error_exit (j_common_ptr cinfo) {
    ...
    // CNVD国家漏洞库通报，精心构造的非法jpg文件，处理unknown markers发生崩溃，参照CxImage中jpeg_error_exit方法，不跳过JERR_UNKNOWN_MARKER错误
...
*/
//  新的turbojpeg 中实现了改方法，看图王集成：注释掉该方法
//static BOOL 
...
jpeg_read_jfxx(FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen) {
	...
	switch(type) {
		case JFXX_TYPE_JPEG:
		{
			...
			FIBITMAP* thumbnail = FreeImage_LoadFromMemory(FIF_JPEG, hmem, 0, NULL);
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack * callback) {
	if (handle) {
        ...
        bool canUseIcc = !((flags & JPEG_NOT_USEICC) == JPEG_NOT_USEICC);
...
#define DSTATE_STOPPING	210
        ...
        bool delColorIcc = false;
		try {
            if (callback && callback->CheckBreakProc && callback->CheckBreakProc(callback->pHandle))
            {
                return NULL;
            }
			...
			if((cinfo.out_color_space == JCS_CMYK) && ((flags & JPEG_CMYK) != JPEG_CMYK)) {
                ...
                // 是否使用lcms纠正编色
                bool useOldConvert = true;
                bool adjustWithLCMS = false;

                if (canUseIcc &&
                    (FreeImage_GetICCProfile(dib)->flags & FIICC_COLOR_IS_CMYK) == FIICC_COLOR_IS_CMYK &&
                    FreeImage_GetBPP(dib) == 32 &&
                    FreeImage_GetColorType(dib) == FIC_CMYK)
                {
                    adjustWithLCMS = true;
                }
                ...
                if (adjustWithLCMS)
                {
                    ...
                    if (profile == NULL || profile->size == 0 || profile->data == NULL)
                    {
                        if (s_pIccProfileData != NULL && s_dIccProfileSize > 0)
                        {
                            inputProfile = cmsOpenProfileFromMem(s_pIccProfileData, s_dIccProfileSize);
                        }
                    }
                    else
                    {
                        inputProfile = cmsOpenProfileFromMem((const void*)profile->data, (cmsUInt32Number)profile->size);
                    }

                    cmsHPROFILE outputProfile = (inputProfile != NULL) ? cmsCreate_sRGBProfile() : NULL;
                    if (inputProfile != NULL &&
                        cmsGetColorSpace(inputProfile) == cmsSigCmykData &&
                        outputProfile != NULL)
                    {
                        cmsUInt32Number inputFormat = TYPE_CMYK_8_REV;

                        // 创建输出profile
                        cmsUInt32Number outputFormat = TYPE_BGRA_8;
                        cmsHPROFILE outputProfile = cmsCreate_sRGBProfile();
                        cmsUInt32Number intent = INTENT_PERCEPTUAL;

                        // 创建转换矩阵
                        cmsUInt32Number dwFlags = 0;
                        hTransform = cmsCreateTransform(inputProfile,
                            inputFormat,
                            outputProfile,
                            outputFormat,
                            intent,
                    ...
                    {
                        useOldConvert = false;
                        LOAD_CHECK_BREAK_BEGIN;
                        while (cinfo.output_scanline < cinfo.output_height)
                        {
                            JSAMPROW dst = FreeImage_GetScanLine(dib, cinfo.output_height - cinfo.output_scanline - 1);
                            LOAD_CHECK_BREAK(cinfo.output_scanline, JPEG_CHECK_DEAL);
                            jpeg_read_scanlines(&cinfo, &dst, 1);
                            /** 之所以传入和传出采用同一个地址原因如下
                            1 通过分析 cmsDoTransform() 的源码可以确认 里面实现从源指针读数据、转换 、写数据到目标指针的操作，都是先读 后写，不会有读写混烂的问题。
                            2 这样做 可以少分配一次内存、少一次所有数据拷贝。
                            3 对时间 空间 上都有利。
                            */
                            cmsDoTransform(hTransform, (const void*)dst, (void*)dst, cinfo.output_width);
                            if ((cinfo.output_scanline % PROGRESS_STEP == 0) && callback && (callback->SetProgressProc != NULL))
                            {
                                callback->SetProgressProc(callback->pHandle, cinfo.output_scanline, cinfo.output_height);
                            }
                        }
                ...
                */
                if (canUseIcc)
                {
                    ...
                    {
                        // 读取
                        cmsHPROFILE inputProfile = cmsOpenProfileFromMem((const void*)profile->data, (cmsUInt32Number)profile->size);
                        if (inputProfile != NULL &&
                            cmsGetColorSpace(inputProfile) == cmsSigRgbData)
                        {
                            const cmsInt32Number descriptionOffset = 12;
                            cmsInt32Number rsize = cmsReadRawTag(inputProfile, cmsSigProfileDescriptionTag, NULL, 0);
                            if (rsize > descriptionOffset)
                            {
                                const char* pDescriptionAdobeRGB = "Adobe RGB (1998)";
                                const size_t descriptionAdobeRGBLen = strlen(pDescriptionAdobeRGB);
                                uint8_t* pBuf = (uint8_t*)malloc(rsize + 1);
                                memset(pBuf, 0, rsize + 1);
                                cmsReadRawTag(inputProfile, cmsSigProfileDescriptionTag, pBuf, rsize);
                                const char* pCurDescriptionRGB = (const char*)(pBuf + descriptionOffset);
                                if (strlen(pCurDescriptionRGB) == descriptionAdobeRGBLen &&
                                    memcmp(pDescriptionAdobeRGB, pCurDescriptionRGB, descriptionAdobeRGBLen) == 0)
                                {
                                    cmsHPROFILE outputProfile = cmsCreate_sRGBProfile();
                                    if (outputProfile != NULL)
                                    {
                                        bool hasAlpha = (cinfo.out_color_space == JCS_EXT_RGBA || cinfo.out_color_space == JCS_EXT_BGRA);
                                        cmsUInt32Number inputFormat = hasAlpha ? TYPE_RGBA_8 : TYPE_RGB_8;
                                        // 创建输出profile
                                        cmsUInt32Number outputFormat = (components == 3) ? TYPE_BGR_8 : TYPE_BGRA_8;
                                        cmsUInt32Number dwFlags = 0;
                                        cmsUInt32Number intent = INTENT_PERCEPTUAL;
                                        hTransform = cmsCreateTransform(inputProfile,
                                            inputFormat,
                ...
                /** CNVD上报问题处理：某些构造的图像cinfo.output_components=8，计算出的bpp大小为64，构造dib时FreeImage_AllocateHeader中将bpp改为8，dib按照此大小构造
                此种情况下，jpeg_read_scanlines时dst大小不够，造成非法访问
                修改方法：先按照row_stride读取数据，然后使用memcpy复制实际大小的数据
                */
                /* JSAMPLEs per row in output buffer */
                int row_stride = cinfo.output_width * cinfo.output_components;
                int scanline_width = ((((FreeImage_GetBPP(dib) * FreeImage_GetWidth(dib)) + 31) / 32) * 4);
                /* Make a one-row-high sample array that will go away when done with image */
				...
				while (cinfo.output_scanline < cinfo.output_height) {
                    ...
                    memcpy(dst, buffer[0], std::min(row_stride, scanline_width));
                ...
                //   DWORD t0 = myGetTickCount();
                if (swapRedBlue)
                {
                    SwapRedBlue32(dib);
                }
                //   myTrace(L"SwapRedBlue: %ums.\n", myGetTickCount() - t0);
#endif
			}
	...
	if ((dib) && (handle)) {
		try {
			...
			if ((flags & JPEG_QUALITYBAD) == JPEG_QUALITYBAD) {
				...
				quality = 75;
            } else if ((flags & JPEG_QUALITYCROP) == JPEG_QUALITYCROP) {
                quality = 95;
            } else if ((flags & JPEG_QUALITYSUPERB) == JPEG_QUALITYSUPERB) {
				quality = 100;
            } else if ((flags & JPEG_QUALITYFREE) == JPEG_QUALITYFREE) {
                quality = flags & 0xFFFF & ~JPEG_QUALITYFREE;
            }
            else {
				if ((flags & 0x7F) == 0) {
					quality = 75;
