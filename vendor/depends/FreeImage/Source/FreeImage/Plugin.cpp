...
FreeImage_LoadFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flags, FreeImage_LoadCallBack* callback) {
	if ((fif >= 0) && (fif < FreeImage_GetFIFCount())) {
		...
		if (node != NULL) {
			if(node->m_plugin->load_proc != NULL) {
				...
				FIBITMAP *bitmap = node->m_plugin->load_proc(io, handle, -1, flags, data, callback);
...
FIBITMAP * DLL_CALLCONV
FreeImage_Load(FREE_IMAGE_FORMAT fif, const char *filename, int flags, FreeImage_LoadCallBack* callback) {
	FreeImageIO io;
	SetDefaultIO(&io);
	...
	if (handle) {
		FIBITMAP *bitmap = FreeImage_LoadFromHandle(fif, &io, (fi_handle)handle, flags, callback);
...
FIBITMAP * DLL_CALLCONV
FreeImage_LoadU(FREE_IMAGE_FORMAT fif, const wchar_t *filename, int flags, FreeImage_LoadCallBack* callback) {
	FreeImageIO io;
	SetDefaultIO(&io);
#ifdef _WIN32	

    // 看图王集成BEGIN
    if (callback && callback->CheckBreakProc(callback->pHandle))
    {
        return NULL;
    }
    // 看图王集成END
