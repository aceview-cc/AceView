...
FreeImage_OpenMultiBitmapU(FREE_IMAGE_FORMAT fif, const wchar_t *filename, BOOL create_new, BOOL read_only, BOOL keep_cache_in_memory, int flags) {
    ...
    if (handle) {
        FIMULTIBITMAP *bitmap = FreeImage_OpenMultiBitmapFromHandle(fif, &io, (fi_handle)handle, flags);

        return bitmap;
    }
    else {
        FreeImage_OutputMessageProc((int)fif, "FreeImage_OpenMultiBitmapU: failed to open input file");
    }
					...
					switch(i->m_type) {
						case BLOCK_CONTINUEUS:
						{
							for (int j = i->getStart(); j <= i->getEnd(); j++) {
								...
								FIBITMAP *dib = header->node->m_plugin->load_proc(&header->io, header->handle, j, header->load_flags, data_read, NULL);
						...
						case BLOCK_REFERENCE:
						{
							...
							FIBITMAP *dib = FreeImage_LoadFromMemory(header->cache_fif, hmem, 0, NULL);
...
FreeImage_CloseMultiBitmap(FIMULTIBITMAP *bitmap, int flags) {
	if (bitmap) {
		...
		if (bitmap->data) {
			...
			if (header != NULL && header->m_decoder != NULL)
			{
				FreeImage_Close(header->node, &header->io, header->handle, header->m_decoder);
				header->m_decoder = NULL;
			}
...
FreeImage_LockPageByHandle(FIMULTIBITMAP* bitmap, int page) {
	if (bitmap) {
		MULTIBITMAPHEADER* header = FreeImage_GetMultiBitmapHeader(bitmap);
		...
		void* data = header->m_decoder;
		if (data != NULL) {
			FIBITMAP* dib = (header->node->m_plugin->load_proc != NULL) ? header->node->m_plugin->load_proc(&header->io, header->handle, page, header->load_flags, data, NULL) : NULL;
		...
		if (node) {
				...
				if (bitmap) {
					...
					if (header) {
						...
						if (fif == FIF_AVIF)
						{
							header->m_decoder = FreeImage_Open(node, &header->io, header->handle, TRUE);
						}
