...
BOOL
ConvertCMYKtoRGBAUseIcc(FIBITMAP* dib, BYTE* pProfileData, int profileSize)
{
    BOOL bRet = FALSE;
    ...
    {
        inputProfile = cmsOpenProfileFromMem((const void*)pProfileData, (cmsUInt32Number)profileSize);
    }
    ...
    // 创建输入profile
    cmsUInt32Number inputFormat = TYPE_CMYK_8;
    // 创建输出profile
    cmsUInt32Number outputFormat = TYPE_BGRA_8;
    if (channelSize == 2)
    {
        inputFormat = TYPE_CMYK_16;
        outputFormat = TYPE_RGBA_16;
    }
    ...
    if (cmsGetColorSpace(inputProfile) != cmsSigCmykData)
    {
        cmsCloseProfile(inputProfile);
    ...
    cmsHTRANSFORM hTransform = cmsCreateTransform(inputProfile,
        inputFormat,
        outputProfile,
        outputFormat,
        intent,
    ...
    if (hTransform != NULL)
    {
        try
        {
            ...
            while (sIndex < height)
            {
                BYTE* dst = FreeImage_GetScanLine(dib, height - sIndex - 1);
                cmsDoTransform(hTransform, (const void*)dst, (void*)dst, width);
                ++sIndex;
            }
            bRet = TRUE;
        }
