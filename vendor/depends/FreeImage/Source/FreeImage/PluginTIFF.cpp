...
ReadThumbnail(FreeImageIO *io, fi_handle handle, void *data, TIFF *tiff, FIBITMAP *dib) {
	...
	if(TIFFGetField(tiff, TIFFTAG_EXIFIFD, &exif_offset)) {
		...
		if(!TIFFLastDirectory(tiff)) {
			...
			thumbnail = Load(io, handle, page, flags, data, NULL);
		...
		if(loadMethod == LoadAsRBGA) {
			...
			if(!header_only) {
				...
				if (samplesperpixel == 4) {
					...
					for (uint32 y = 0; y < height; y++) {
                        LOAD_CHECK_BREAK(y, TIFF_CHECK_DEAL(raster));

                        if ((y % PROGRESS_STEP == 0) && callback && (callback->SetProgressProc != NULL))
                        {
                            callback->SetProgressProc(callback->pHandle, y, height);
                        }
			...
			if(planar_config == PLANARCONFIG_CONTIG && !header_only) {
                ...
                for (uint32 y = 0; y < height; y += rowsperstrip) {
                    LOAD_CHECK_BREAK(y, TIFF_CHECK_DEAL(buf));
			...
			if(!header_only) {
				...
				if(!asCMYK) {
                    if (!ConvertCMYKtoRGBAUseIcc(dib, (BYTE *)iccBuf, (int)iccSize))
                    {
			...
			if(planar_config == PLANARCONFIG_CONTIG && !header_only) {
				...
				for (uint32 y = 0; y < height; y += tileHeight) {						
                    ...
                    LOAD_CHECK_BREAK(y, TIFF_CHECK_DEAL(tileBuffer));
