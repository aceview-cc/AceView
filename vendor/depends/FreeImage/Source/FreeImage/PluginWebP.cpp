...
ReadFileToWebPData(FreeImageIO *io, fi_handle handle, WebPData * const bitstream) {
  ...
  try {
	  ...
	  raw_data = (uint8_t*)malloc(file_length * sizeof(uint8_t) + 1);
	  if(!raw_data) {
      ...
      raw_data[file_length * sizeof(uint8_t)] = '\0';
...
static BOOL
FreeImage_SetMetadataWebP(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, const char *key, WORD id, FREE_IMAGE_MDTYPE type, DWORD count, DWORD length, const void *value)
{
    BOOL bResult = FALSE;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data, FreeImage_LoadCallBack * callback) {
	...
	try {
		...
		// get image data
		error_status = WebPMuxGetFrame(mux, page, &webp_frame);
		...
		if(error_status == WEBP_MUX_OK) {
            ...
            {
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "Width", ANIMTAG_LOGICALWIDTH, FIDT_LONG, 1, 4, &width);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "Height", ANIMTAG_LOGICALHEIGHT, FIDT_LONG, 1, 4, &height);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "FrameLeft", ANIMTAG_FRAMELEFT, FIDT_LONG, 1, 4, &webp_frame.x_offset);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "FrameTop", ANIMTAG_FRAMETOP, FIDT_LONG, 1, 4, &webp_frame.y_offset);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "FrameDispose", ANIMTAG_DISPOSALMETHOD, FIDT_SHORT, 1, 2, &webp_frame.dispose_method);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "FrameBlend", ANIMTAG_BLENDMETHOD, FIDT_SHORT, 1, 2, &webp_frame.blend_method);
                FreeImage_SetMetadataWebP(FIMD_ANIMATION, dib, "FrameDuration", ANIMTAG_DURATION, FIDT_LONG, 1, 4, &webp_frame.duration);
            }
...
PageCount(FreeImageIO *io, fi_handle handle, void *data) {
    ...
    if (mux != NULL) {
        ...
        int nFrames = 1;
        WebPMuxError err = WebPMuxNumChunks(mux, id, &nFrames);
        if (err == WEBP_MUX_OK) {
            return nFrames;
        }
    }
