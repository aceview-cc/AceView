...
// 看图王集成BEGIN
BITMAPINFO* DLL_CALLCONV FreeImage_CreateICCProfileHeaderData(FIBITMAP* dib, void* data, long size)
{
    if (data == NULL || size <= 0)
    {
        return NULL;
    ...
    BITMAPV5HEADER bitmapHeadProfiled = { sizeof(BITMAPV5HEADER) };
    bitmapHeadProfiled.bV5Width = FreeImage_GetWidth(dib);
    bitmapHeadProfiled.bV5Height = FreeImage_GetHeight(dib);
    bitmapHeadProfiled.bV5Planes = 1;
    bitmapHeadProfiled.bV5BitCount = FreeImage_GetBPP(dib);
    bitmapHeadProfiled.bV5Compression = BI_RGB;
    bitmapHeadProfiled.bV5SizeImage = 0;
    bitmapHeadProfiled.bV5CSType = PROFILE_EMBEDDED;
    bitmapHeadProfiled.bV5Intent = LCS_GM_IMAGES;
    bitmapHeadProfiled.bV5ProfileData = sizeof(BITMAPV5HEADER);
    bitmapHeadProfiled.bV5ProfileSize = size;

    DWORD nBitmapHeadSize = sizeof(BITMAPV5HEADER) + bitmapHeadProfiled.bV5ProfileSize;
    BYTE* pBitmapHeader = (BYTE*)malloc(nBitmapHeadSize);
    if (pBitmapHeader == NULL)
    {
        return NULL;
    }

    BYTE* pBitmapHeaderBuffer = pBitmapHeader;
    memcpy(pBitmapHeaderBuffer, &bitmapHeadProfiled, sizeof(BITMAPV5HEADER));
    pBitmapHeaderBuffer += sizeof(BITMAPV5HEADER);
    memcpy(pBitmapHeaderBuffer, data, bitmapHeadProfiled.bV5ProfileSize);

    return (BITMAPINFO*)pBitmapHeader;
}

DLL_API void DLL_CALLCONV FreeImage_DestroyICCProfileHeaderData(BITMAPINFO* pICCHeader)
{
    BYTE* pData = (BYTE*)pICCHeader;
    if (pData)
    {
        free(pData);
    }
...
{
    if (srcDIB == NULL || destDIB == NULL)
    {
        return FALSE;
    }

    FIICCPROFILE* src_iccProfile = FreeImage_GetICCProfile(srcDIB);
    FIICCPROFILE* dst_iccProfile = FreeImage_GetICCProfile(destDIB);
    memset(dst_iccProfile, 0, sizeof(FIICCPROFILE));
    FreeImage_CreateICCProfile(destDIB, src_iccProfile->data, src_iccProfile->size);
    dst_iccProfile->flags = src_iccProfile->flags;
