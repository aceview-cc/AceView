...
int psdThumbnail::Read(FreeImageIO *io, fi_handle handle, int iResourceSize, bool isBGR, int flags) {
	BYTE ShortValue[2], IntValue[4];
	int nBytes=0, n;
	...
	if(_Format == 1) {
        ...
        bool notUseIcc = ((flags & PSD_NOT_USEICC) == PSD_NOT_USEICC);
        _dib = FreeImage_LoadFromHandle(FIF_JPEG, io, handle, notUseIcc ? JPEG_NOT_USEICC : 0, NULL);
		if(isBGR) {
			SwapRedBlue32(_dib);
		}
...
bool psdParser::ReadLayerAndMaskInfoSection(FreeImageIO *io, fi_handle handle)	{
    ...
    if (_headerInfo._Version == 1)
    {
        ...
        if (n)
        {
            ...
            if (nTotalBytes >= 0)
            {
                bSuccess = io->seek_proc((FILE *)handle, nTotalBytes, SEEK_CUR) != -1;
            }
        }
    }
    else if (_headerInfo._Version == 2)
    {
        // psb file
        bSuccess = true;
        ...
        if (sizeof(long) < sizeof(UINT64)) {
            ...
            while (nTotalBytes > offset) {
                if (io->seek_proc(handle, offset, SEEK_CUR) == -1) {
                    bSuccess = false;
                    break;
                }
                nTotalBytes -= offset;
        ...
        if (bSuccess && nTotalBytes > 0) {
            if (io->seek_proc(handle, (long)nTotalBytes, SEEK_CUR) == -1)
                bSuccess = false;
        }
    }
			...
			if ( 0 < oResource._Size ) {
				...
				switch( oResource._ID ) {
					...
					case PSDP_RES_THUMBNAIL:
					{
						...
						nBytes += _thumbnail.Read(io, handle, oResource._Size, bBGR, _fi_flags);
						break;
					}
...
void psdParser::UnpackRLE(BYTE* line, const BYTE* rle_line, BYTE* line_end, unsigned srcSize) {
	while (srcSize > 0) {
		...
		if ( len < 128 ) { //<- MSB is not set
            ...
            /* memcpy的大小可能是line_end-line，之后line+=len后，line_end-line已经小于0，提前退出 */
            if (line_end - line <= 0) {
                return;
            }
            ...
            /* memset的大小可能是line_end-line，之后line+=len后，line_end-line已经小于0，提前退出 */
            if (line_end - line <= 0) {
                return;
            }
...
FIBITMAP* psdParser::ReadImageData(FreeImageIO *io, fi_handle handle) {
	...
	if((mode == PSDP_CMYK || mode == PSDP_MULTICHANNEL)) {
		...
		if((_fi_flags & PSD_CMYK) == PSD_CMYK) {
			...
			// convert to RGB
            bool notUseIcc = ((_fi_flags & PSD_NOT_USEICC) == PSD_NOT_USEICC);
            if (notUseIcc ||
                !ConvertCMYKtoRGBAUseIcc(bitmap, _iccProfile._ProfileData, _iccProfile._ProfileSize))
            {
            ...
            if (nChannels == 4 || nChannels == 3 || nChannels == 5) {
				FIBITMAP* t = RemoveAlphaChannel(bitmap);
				if(t) {
					FreeImage_Unload(bitmap);
					bitmap = t;
				}
                else // else: silently fail
                {
                    FreeImage_GetICCProfile(bitmap)->flags |= FIICC_COLOR_IS_CMYK;
                }
			}
