...
// 看图王集成BEGIN
// =====================================================================
// Fake Memory IO functions
// =====================================================================

unsigned DLL_CALLCONV
_FakeMemoryWriteProc(void *buffer, unsigned size, unsigned count, fi_handle handle) {
    void *newdata = NULL;
    long newdatalen;
...
void
SetFakeMemoryIO(FreeImageIO *io) {
    io->read_proc = _MemoryReadProc;
    io->seek_proc = _MemorySeekProc;
    io->tell_proc = _MemoryTellProc;
    io->write_proc = _FakeMemoryWriteProc;
}
