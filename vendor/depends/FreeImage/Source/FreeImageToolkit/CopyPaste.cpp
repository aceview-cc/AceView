...
/// 32- bit
static BOOL Combine32(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha, BOOL modifyDstAlpha = TRUE);
// ----------------------------------------------------------
...
static BOOL 
Combine32(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha, BOOL modifyDstAlpha) {
	// check the bit depth of src and dst images
	...
	if (alpha > 255) {
		...
		// alpha blend images
		for(unsigned rows = 0; rows < FreeImage_GetHeight(src_dib); rows++) {
			for(unsigned cols = 0; cols < FreeImage_GetLine(src_dib); cols+=4) {
                if (!modifyDstAlpha && src_bits[cols + 3] == 0) {
                    continue;
                }

                dst_bits[cols] = (BYTE)(((src_bits[cols] - dst_bits[cols]) * alpha + (dst_bits[cols] << 8)) >> 8);
                dst_bits[cols + 1] = (BYTE)(((src_bits[cols + 1] - dst_bits[cols + 1]) * alpha + (dst_bits[cols + 1] << 8)) >> 8);
                dst_bits[cols + 2] = (BYTE)(((src_bits[cols + 2] - dst_bits[cols + 2]) * alpha + (dst_bits[cols + 2] << 8)) >> 8);
                dst_bits[cols + 3] = (BYTE)(((src_bits[cols + 3] - dst_bits[cols + 3]) * alpha + (dst_bits[cols + 3] << 8)) >> 8);
			}
...
*/
BOOL DLL_CALLCONV 
FreeImage_Paste(FIBITMAP *dst, FIBITMAP *src, int left, int top, int alpha, BOOL modifyDstAlpha) {
	BOOL bResult = FALSE;
	...
	if(image_type == FIT_BITMAP) {
		...
		switch(FreeImage_GetBPP(dst)) {
			...
			case 24:
				...
				break;
			case 32:
				bResult = Combine32(dst, clone, (unsigned)left, (unsigned)top, (unsigned)alpha, modifyDstAlpha);
				break;
		}
